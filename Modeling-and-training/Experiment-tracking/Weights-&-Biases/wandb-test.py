...
# To log anything to the wandb.ai server, you need to login first, and if using the local wandb server, then you also
# need to start that, with "wandb server start" in a terminal. Either you can log in with "wand.login()"
# and then navigate to wandb.ai/authorize to copy your API key, so that you can paste it when promted in the terminal,
# or, if you already have your API key, you can login with "wandb.login(key='your key goes here')". The key here
# is a string and must go inside quotes.
wandb.login()
